    ...
    <form action="{{ route('employees.update',$employee) }}" method="post">
        ...
        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
            ...
            <div class="sm:col-span-4">
                ...
                <div class="mt-2">
                    <input id="email" value="{{old('email',$employee->email)}}" name="email" type="email" autocomplete="email" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                </div>
                @error('email')
                <span class="text-xs text-red-600 dark:text-red-400">
                      {{$message}}
                ...
                <div class="mt-2">
                    <select id="company" name="company_id" autocomplete="company-name"
                        ...
                        @foreach($companies as $company)
                            <option value="{{$company->id}}" @selected(old('company',$employee->company_id) == $company->id)>{{$company->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
